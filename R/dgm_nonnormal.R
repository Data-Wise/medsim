...
#' @param skew Target marginal skewness (third standardized moment).
#' @param kurtosis Target marginal excess kurtosis (fourth standardized moment minus 3).
...
.medsim_fleishman_coef <- function(skew, kurtosis) {
  ...
  if (kurtosis <= skew^2 - 2) {
    stop(sprintf(
      paste0(
        "Infeasible (skew = %g, kurtosis = %g): excess kurtosis must ",
        "exceed skew^2 - 2 = %g."
      ),
      skew, kurtosis, skew^2 - 2
    ), call. = FALSE)
  }
  ...
  if (!converged || any(!is.finite(p))) {
    stop(sprintf(
      paste0(
        "Could not solve Fleishman coefficients for (skew = %g, ",
